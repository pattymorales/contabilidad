    ...
    public partial class FPlanCuentas : Form
    {
            ...
            { 
                if (nivel == 1)
                {
                    ...
                    foreach (Cuenta CuentaRow in acount)
                    {
                        ...
                        if (var.Length == nivC)
                        {
                            treeView1.Nodes.Add (CuentaRow.cuenta, CuentaRow.cuenta + " - " + CuentaRow.descripcion  );
                        }
                    }
                }
                if (nivel > 1)
        ...
        private void InsertCuentas( TreeView node, Cuenta CuentaRow,  Int16 nivel, Int32 nivelLenght, Int32 nivelPadreLenght)
        {
            ...
            foreach (TreeNode n in myNodes)
            {
                ...
                if (padre == n.Name)
                {
                    node.Nodes[n.Index].Nodes.Add(CuentaRow.cuenta, CuentaRow.cuenta + " - " + CuentaRow.descripcion);
                }
                else
                {
